...
def get_line_endpoints(line: Tuple[float, float, float, float]) -> Tuple[Tuple[float, float], Tuple[float, float]]:
    x1, y1, x2, y2 = line
    return (x1, y1), (x2, y2)
...
def are_lines_almost_connected(line1: Tuple[float, float, float, float], 
...
def find_bounding_boxes_with_orientation_constraint(lines: List[Tuple[float, float, float, float]], 
                                                   threshold: float = 10.0) -> List[List[Tuple[float, float, float, float]]]:
    boxes = []
    ...
    for i in range(len(lines)):
        for j in range(i+1, len(lines)):
            for k in range(j+1, len(lines)):
                ...
                orient1, orient2, orient3 = orientations[i], orientations[j], orientations[k]
                ...
                if (are_lines_almost_connected(line1, line2, threshold) and
                    are_lines_almost_connected(line2, line3, threshold) and
                    are_lines_almost_connected(line1, line3, threshold)):
        ...
        for j in range(i+1, len(lines)):
            for k in range(j+1, len(lines)):
                for l in range(k+1, len(lines)):
                    ...
                    orient1, orient2, orient3, orient4 = orientations[i], orientations[j], orientations[k], orientations[l]
                    
                    # Check if orientations are compatible (one vertical, one horizontal)
                    if not (are_orientations_compatible(orient1, orient2) and 
                            are_orientations_compatible(orient2, orient3) and 
                            are_orientations_compatible(orient3, orient4) and 
                            are_orientations_compatible(orient4, orient1)):
                        continue
                    
                    # Check if lines form a closed loop
                    if (are_lines_almost_connected(line1, line2, threshold) and
                        are_lines_almost_connected(line2, line3, threshold) and
                        are_lines_almost_connected(line3, line4, threshold) and
                        are_lines_almost_connected(line4, line1, threshold)):
...
def calculate_line_angle(line: Tuple[float, float, float, float], roundTo = 1):
    """
    ...
    return round_number(abs(angle), roundTo)
...
def find_bounding_boxes_with_two_connections(lines: List[Tuple[float, float, float, float]], 
                                            threshold: float = 10.0) -> List[List[Tuple[float, float, float, float]]]:
    boxes = []
    ...
    for i in range(len(lines)):
        for j in range(i+1, len(lines)):
            for k in range(j+1, len(lines)):
                ...
                #         are_orientations_compatible(orient1, orient3)):
                #     continue
                if abs(calculate_line_angle(line1, 5) - calculate_line_angle(line3, 5)) <= 10:
                    connections += 1
                if abs(calculate_line_angle(line2, 5) - calculate_line_angle(line1, 5)) <= 10:
                    connections += 1
                if abs(calculate_line_angle(line3, 5) - calculate_line_angle(line2, 5)) <= 10:
                    connections += 1
                #connections = 5
                # For a valid partial rectangle, we need at least 2 connections
                if connections < 1:
